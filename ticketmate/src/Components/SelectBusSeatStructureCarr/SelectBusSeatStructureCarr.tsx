...
import './SelectBusSeatStructureCarr.css';
...
const SelectBusSeatStructureCarr: React.FC<SelectBusSeatStructureProps> = ({ buttonStates, setButtonStates }) => {
  ...
  const handleButtonClick = (seatId: string) => {
    setButtonStates(prevState => ({
      ...
      [seatId]: {
        ...
        id: prevState[seatId]?.id ?? -1, // Ensure the ID is not -1
      }
    }));
  };
...
export default SelectBusSeatStructureCarr;
