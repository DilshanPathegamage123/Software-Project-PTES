...
function Driver2() {
  ...
  const handleend = () => {
    if (travelDetails.routNo) {
      axios
        .put(
          `https://localhost:7296/api/ScheduledBus/endbustrip/${travelDetails.id}`,
          {
            headers: {
      ...
      axios
        .put(
          `https://localhost:7296/api/Scheduledtrain/endtraintrip/${travelDetails.id}`,
          {
            headers: {
              Authorization: `Bearer ${getToken()}`,
            },
  ...
  const passengerIds = Array.from({ length: 100 }, (_, i) => i + 50); // Creates an array [50, 51, ..., 100]
        ...
        <div className="container">
          ...
          {travelDetails.routNo ? (
            <div className="row fs-5 mt-2">
              Vehicle No: {travelDetails.busNo}
            </div>
          ) : (
            <div className="row fs-5 mt-2">
              Train Name: {travelDetails.trainName}
            </div>
          )}
          ...
          <div
            className="p-5 rounded-4 custom-height"
            style={{ background: "#FFFFFF" }}
          >
            <StartRideButton rideId={travelDetails.id} />
            &nbsp;&nbsp;
            <span onClick={handleend}>
              <EndRideButton rideId={travelDetails.id} connectionId="" />
            </span>
            <VehicleLocation rideId={travelDetails.id} />
          </div>
        </div>
        <UpdateBreakdown
          routNo={travelDetails.routNo}
          Id={travelDetails.id}
          DriverId={DriverId}
          BusNo={travelDetails.busNo}
        />
      </div>
      <Footer />
